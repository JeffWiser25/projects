...
#define IR_LED 1      // Infrared LED (optional, not used in logic)
#define IR_SENS A0    // Infrared sensor input pin (analog)
#define RED_LED 5     // LED indicator for blade detection
...
void setup() {
  pinMode(IR_LED, OUTPUT);   // Set IR LED pin as output (optional, not used in logic)
  pinMode(IR_SENS, INPUT);   // Set IR sensor pin as input
  pinMode(RED_LED, OUTPUT);  // Set LED indicator pin as output

  Serial.begin(115200);  // Start serial communication at 115200 baud rate (for faster data transfer)
...
void loop() {
  ...
  bool bladeDetected = (sensorValue > 150);  // Blade detected if sensor value exceeds threshold (adjust if needed)
  ...
  if (bladeDetected && !prevBladeState && millis() - lastBladeTime > debounceTime) {
    ...
    digitalWrite(RED_LED, HIGH);  // Turn on LED indicator
    lastBladeTime = millis();  // Update last detection time
  } 
  else if (!bladeDetected) {
    digitalWrite(RED_LED, LOW);  // Turn off LED if no blade is detected
  }
