...
int main(int argc, char *argv[]) {
    ...
    // Counters for ring and solid images
    int ring_count = 0;
    int solid_count = 0;
    ...
    while ((entry = readdir(dir)) != NULL) { 
        ...
        if (strstr(entry->d_name, ".pgm") != NULL) {
            ...
            if (is_ring) { // If classified as ring
                ring_count++; // Increment ring count
                snprintf(output_path, sizeof(output_path), "%s/ring_%d.pgm", rings_dir, ring_count);
                printf("Classified %s as ring. Moving to '%s'.\n", entry->d_name, rings_dir);
            } else {       // If classified as solid
                solid_count++; // Increment solid count
                snprintf(output_path, sizeof(output_path), "%s/solid_%d.pgm", solids_dir, solid_count);
                printf("Classified %s as solid. Moving to '%s'.\n", entry->d_name, solids_dir);
            }

            // Move the file to the appropriate directory with new naming
    ...
    // Print total counts of rings and solids sorted
