...
// Define pin numbers
#define IR_LED 1    // Infra-red LED
#define IR_SENS A0  // Assuming the IR sensor is connected to an analog pin
#define RED_LED 5   // Optional red LED being used as counter
...
void setup() {
  ...
  Serial.println("LABEL,TIME,Millis,WindSpeed_km_h,BladeCount");
  // # For use with PLX-DAQ v.2
...
//   # If your blades are lighter than the background, detecting not black (sensorValue > 200) could be better.
  ...
  if (elapsedTime >= 1.0) {  // Update every 1 second
    ...
    // Printing for use with PLX-DAQ v.2.11
    Serial.print("DATA,TIME,");  // "TIME" makes PLX-DAQ auto-fill real time
    Serial.print(millis());
    Serial.print(",");
    Serial.print(compensated_wind_speed, 2);
    Serial.print(",");
    Serial.println(counter);
